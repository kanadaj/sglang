...
class DefaultModelLoader(BaseModelLoader):
    ...
    def _get_all_weights(
        ...
        # Resolve once, before model.load_weights can pre-read checkpoint metadata.
        # Keep identity on the model, not config/server globals (target and draft
        # may use different revisions, cache directories or local checkpoints).
        resolved_sources = self.resolve_model_weights(model_config, model)
        model._resolved_weight_sources = resolved_sources

        def weights_iterator():
            for resolved_source in resolved_sources:
                yield from self._get_weights_iterator(
                    resolved_source.source, resolved_source=resolved_source
                )
