...
class Qwen4ExpForCausalLMMTP(Qwen3_5ForCausalLMMTP):
    ...
    def set_lm_head_from_target(self, target_lm_head):
        from sglang.srt.layers.quantization.private_draft_head import select_draft_head

        if self.config.tie_word_embeddings:
            return super().set_lm_head_from_target(target_lm_head)
        # Called after embedding sharing, before draft attention/graph setup.
        # Conversion owns a separate module and never writes the target weight.
        self.lm_head = select_draft_head(target_lm_head)
