    ...
    def get_quant_method(
        ...
        if isinstance(layer, (LinearBase, ParallelLMHead)):
            ...
            if quant_algo == "MXFP8" and prefix.endswith(".in_proj_ba"):
                ...
                return UnquantizedLinearMethod()
            if quant_algo == "MXFP8":
                if prefix.startswith(("model.visual.blocks.", "visual.blocks.")) and prefix.endswith(".mlp.linear_fc1"):
                    from sglang.srt.layers.quantization.vision_mxfp8 import VisionMxfp8PaddedLinearMethod
                    return VisionMxfp8PaddedLinearMethod(self.mxfp8_config)
                return Fp8LinearMethod(self.mxfp8_config)
            if quant_algo == "NVFP4":
                return ModelOptFp4LinearMethod(self.nvfp4_config)
            if quant_algo == "W4A16_NVFP4":
                if prefix.startswith(("model.visual.blocks.", "visual.blocks.")) and prefix.endswith(".mlp.linear_fc2"):
                    from sglang.srt.layers.quantization.vision_mxfp8 import VisionNvFp4A16LinearMethod
                    return VisionNvFp4A16LinearMethod(self.nvfp4a16_config)
