        ...
        for payload in event_payloads(events):
            if (
                ...
                if payload["type"] == "response.output_item.added":
                    self.assertIsNone(payload["item"]["phase"])
                else:
                    self.assertEqual(payload["item"], output[payload["output_index"]])
        self.assertEqual(
            [item.model_dump() for item in serving.response_store[request.request_id].output],
            output,
        )
        replay_messages = [
            item for item in serving.msg_store[request.request_id]
            if item.get("type") == "message" and item.get("role") == "assistant"
        ]
        self.assertEqual([item["phase"] for item in replay_messages], ["commentary", "final_answer"])
...
class MultiToolCallStreamingOrderTestCase(CustomTestCase):
    ...
    def _seq(self, texts, *names):
        ...
        seq = StreamFixture(self.serving, request).run_seq(chunks)
        output = next(p["response"]["output"] for t, p in seq if t == "response.completed")
        for t, p in seq:
            if t == "response.output_item.added" and p["item"]["type"] == "message":
                self.assertIsNone(p["item"]["phase"])
            if t == "response.output_item.done":
                self.assertEqual(p["item"], output[p["output_index"]])
                if p["item"]["type"] == "message":
                    self.assertEqual(p["item"]["phase"], "commentary")
        return seq
