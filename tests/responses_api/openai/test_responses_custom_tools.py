...
class NamespaceDeclarationTestCase(CustomTestCase):
    def test_named_member_reaches_prompt_processing(self):
        for stream in (False, True):
            for choice in (
                {"type": "function", "namespace": "weather", "name": "lookup"},
                {"type": "function", "name": "weather.lookup"},
            ):
                with self.subTest(stream=stream, choice=choice):
                    serving = make_serving()
                    # Stop before inference; exercise the public handler's validation.
                    serving._make_request = AsyncMock(
                        side_effect=ValueError("prompt processing reached")
                    )
                    request = _namespace_request(tool_choice=choice, stream=stream)
                    response = asyncio.run(serving.create_responses(request))
                    serving._make_request.assert_awaited_once()
                    self.assertIn(b"prompt processing reached", response.body)

    def test_named_member_rejects_unknown_namespace_or_member(self):
        for namespace, name in (("weather", "missing"), ("missing", "lookup")):
            for stream in (False, True):
                serving = make_serving()
                ...
                self.assertIn(b"not declared", response.body)
                serving.tokenizer_manager.generate_request.assert_not_called()
